...
class VideoService {
  private readonly baseUrl = "/api/v1/posts/categories/videos";
  ...
  async getVideoById(id: string): Promise<Video> {
    try {
      const response = await axios.get<Video>(`/api/v1/posts/${id}`);
      return response.data;
    } catch (error) {
  ...
  async getVideoBySlug(slug: string): Promise<Video> {
    try {
      const response = await axios.get<Video>(`/api/v1/posts/slug/${slug}`);
      return response.data;
    } catch (error) {
  ...
  async incrementViewCount(id: string): Promise<void> {
    try {
      await axios.post(`/api/v1/posts/${id}/view`);
    } catch (error) {
  ...
  async unlikeVideo(id: string): Promise<void> {
    try {
      await axios.delete(`/api/v1/posts/${id}/like`);
    } catch (error) {
